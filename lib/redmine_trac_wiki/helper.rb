  ...
  module Helper
    ...
    def wikitoolbar_for(field_id)
      ...
      javascript_include_tag('jstoolbar/jstoolbar') +
        javascript_include_tag('trac', :plugin => 'redmine_trac_wiki') +
        javascript_include_tag("lang/trac-#{current_language}", :plugin => 'redmine_trac_wiki') +
        javascript_tag(<<-EOS + (Setting.plugin_redmine_trac_wiki[:trac_wiki_require_block].to_s == 'true' ? <<-EOT : ''))
          var editor = $('#{field_id}');
      ...
      unless @heads_for_wiki_formatter_included
        content_for :header_tags do
          stylesheet_link_tag('jstoolbar.css') +
            stylesheet_link_tag('trac.css', :plugin => 'redmine_trac_wiki')
        end
